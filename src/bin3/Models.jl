...
function mst!(genv, WD, GD, xH, pAH) # Now the master problem has Common = 0
    ...
    JuMP.@variable(m, θ[1:S]); N = 0; for (z,Ø)=enumerate(GDˈn), (g,node)=enumerate(Ø)
        @inbounds(begin
    ...
    end; Xl=fill(0.,N); Bv=fill(Cchar('C'),N); Θ=fill(-Inf,S); nX=2
    ...
    Vv = CircularBuffer(fill(SB_CUT_COT, S)) # violations
    (; m, o, refd, refi, S, N, Θ, Xl, Xn, Bv, Vv)
end
...
function sub!(Ve, s, T, genv, t, LineˈP, F, WD, LD, GD, xH, pAH; ReserveK2D=0.04, reserve_type=1, ϖCost=50, ζCost=150)
    ...
    Ci, Cd = Cint[range(S; length=N); s-1], Vector{Cdouble}(undef, N+2)
